...
namespace BiliFor.Api.Controllers
{
    ...
    public class BiLiBiLiLoginController : Controller
    {
        ...
        public BiLiBiLiLoginController(IVipPrivilegeDomainService vipService, IBiliCookieServices bilicookieservices, IBiliAccountService biliaccountservice, IMangaDomainService managadomainservice, ILogger<BiLiBiLiLoginController> logger)
        {
        ...
        public async Task<string> GetUserInfo()
        {
            ...
            //获取登录用户信息
            BiliUserInfo biliuserinfo = _biliaccountservice.LoginByCookie(cookie);

            //获取每日任务
            DailyTaskInfo daily = _biliaccountservice.GetDailyTaskStatus(cookie);

            //领取大会员权益
            _vipService.ReceiveVipPrivilege(biliuserinfo, cookieentity);
